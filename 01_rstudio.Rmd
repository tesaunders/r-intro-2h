...
Let's move the workshop [data file](data_raw/surveys_complete.csv) into the data_raw folder now. Next we need to unzip the file so that the 5 files inside are now sitting in the data_raw folder. You should see a file called `surveys_complete.csv` which is the data we will use first. You should also see this same data file split into 4 separate spreadsheet files, which we'll use later to demonstrate how to read multiple files in at once.
...
R represents this data in particular ways, and the way it represents it determines what operations can be done with it.

The two main data types you'll see are:

- _numeric_, and
- _character_

We've seen numeric data already e.g. `100`, or `2.2`. 

The character data type (text data) are zero or more characters (e.g. letters, numerals, punctuation, etc) that are enclosed in either single (`'`) or double (`"`) quotation marks -- whichever you open with you need to close with. With character data you can't do mathematical operations. Where this becomes especially important is when reading data into R, making sure that R represents it correctly.
...
We can see how R is representing the data (the data type) by using `class`
...
With numeric data you can do the usual mathematical operations.
...
Whereas, even though it looks like the number `6`, because it is enclosed in quotes it is treated as a character datatype and so mathematical operations on characters don't make sense and you get an error.
...
The third common data type you will come across is the _logical_ (a.k.a. boolean), which is the values `TRUE` or `FALSE` and is usually the output of a conditional statement (which we'll see later on).

# Vectors

Vectors are 1-dimensional series of values, and vectors are the basic building blocks of all data in R. When we assigned values to objects just now we were creating vectors. All items in a vector must be of the same data type. 

To create a vector with multiple values, we can use the `c()` function, putting values inside, separated by commas.

```{r}
age <- c(1, 2, 5, 12, 4)
fruit <- c("apple", "pear", "grape")
```
...
There is an important distinction between 0 and a missing value. A 0 means a 0 was observed, whereas a missing value means there was no observation recorded. R represents missing data as NA, without quotes, in all vector types.

```{r}
weights <- c(25, 34, 12, NA, 42)
```

R doesn’t make assumptions about how you want to handle missing data, so if we pass this vector to a numeric function like `mean()` to get the minimum value, it won’t know what to do, so it returns NA.
